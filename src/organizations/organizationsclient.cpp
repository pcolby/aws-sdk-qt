 ...
 * named <i>OrgsServiceLinkedRoleName</i>. For more information, see <a
 ...
 * operation>
 *
 * The user who calls the API for an invitation to join must have the <code>organizations:CreateAccount</code> permission.
 * If you enabled all features in the organization, then the user must also have the
 * <code>iam:CreateServiceLinkedRole</code> permission so that Organizations can create the required service-linked role
 * named <i>OrgsServiceLinkedRoleName</i>. For more information, see <a
 * href="http://docs.aws.amazon.com/organizations/latest/userguide/orgs_integration_services.html#orgs_integration_service-linked-roles">AWS
 * Organizations and Service-Linked Roles</a> in the <i>AWS Organizations User
 ...
 * Guide</i>>
 *
 * The user in the master account who calls this API must also have the <code>iam:CreateRole</code> permission because AWS
 ...
 * Guide</i>> </li> <li>
 *
 * If you get an exception that indicates that you exceeded your account limits for the organization or that the operation
 * failed because your organization is still initializing, wait one hour and then try again. If the error persists after an
 * hour, then contact <a href="https://console.aws.amazon.com/support/home#/">AWS Customer
 *
 * Support</a>> </li> <li>
 *
 * Because <code>CreateAccount</code> operates asynchronously, it can return a successful completion message even though
 ...
 * permissions can view billing information for the account. If you disable this, then only the account root user can
 * access billing information. For information about how to disable this for an account, see <a
