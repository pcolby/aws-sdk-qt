...
#include "closeaccountresponse_p.h"
...
/*!
 * \class QtAws::Organizations::CloseAccountResponse
 * \brief The CloseAccountResponse class provides an interace for Organizations CloseAccount responses.
 *
 ...
 *
 * \sa OrganizationsClient::closeAccount
 */

/*!
 * Constructs a CloseAccountResponse object for \a reply to \a request, with parent \a parent.
 */
CloseAccountResponse::CloseAccountResponse(
        const CloseAccountRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : OrganizationsResponse(new CloseAccountResponsePrivate(this), parent)
{
    setRequest(new CloseAccountRequest(request));
    setReply(reply);
}
 ...
 * \reimp
 */
const CloseAccountRequest * CloseAccountResponse::request() const
{
    Q_D(const CloseAccountResponse);
    return static_cast<const CloseAccountRequest *>(d->request);
}

/*!
 * \reimp
 * Parses a successful Organizations CloseAccount \a response.
 */
void CloseAccountResponse::parseSuccess(QIODevice &response)
{
    //Q_D(CloseAccountResponse);
    QXmlStreamReader xml(&response);
    /// @todo
}

/*!
 * \class QtAws::Organizations::CloseAccountResponsePrivate
 * \brief The CloseAccountResponsePrivate class provides private implementation for CloseAccountResponse.
 * \internal
 *
 * \inmodule QtAwsOrganizations
 */
...
/*!
 * Constructs a CloseAccountResponsePrivate object with public implementation \a q.
 */
CloseAccountResponsePrivate::CloseAccountResponsePrivate(
    CloseAccountResponse * const q) : OrganizationsResponsePrivate(q)
{

}

/*!
 * Parses a Organizations CloseAccount response element from \a xml.
 */
void CloseAccountResponsePrivate::parseCloseAccountResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("CloseAccountResponse"));
    Q_UNUSED(xml) ///< @todo
}
