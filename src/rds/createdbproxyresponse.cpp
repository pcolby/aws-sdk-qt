...
#include "createdbproxyresponse_p.h"
...
/*!
 * Constructs a CreateDBProxyResponse object for \a reply to \a request, with parent \a parent.
 */
CreateDBProxyResponse::CreateDBProxyResponse(
        const CreateDBProxyRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : RdsResponse(new CreateDBProxyResponsePrivate(this), parent)
{
    setRequest(new CreateDBProxyRequest(request));
    setReply(reply);
}
...
/*!
 * Constructs a CreateDBProxyResponsePrivate object with public implementation \a q.
 */
CreateDBProxyResponsePrivate::CreateDBProxyResponsePrivate(
    CreateDBProxyResponse * const q) : RdsResponsePrivate(q)
{
...
{
    Q_ASSERT(xml.name() == QLatin1String("CreateDBProxyResponse"));
    Q_UNUSED(xml) ///< @todo
}
