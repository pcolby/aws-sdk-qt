...
#include "modifydbproxyresponse_p.h"
...
/*!
 * Constructs a ModifyDBProxyResponse object for \a reply to \a request, with parent \a parent.
 */
ModifyDBProxyResponse::ModifyDBProxyResponse(
        const ModifyDBProxyRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : RdsResponse(new ModifyDBProxyResponsePrivate(this), parent)
{
    setRequest(new ModifyDBProxyRequest(request));
    setReply(reply);
}
...
/*!
 * Constructs a ModifyDBProxyResponsePrivate object with public implementation \a q.
 */
ModifyDBProxyResponsePrivate::ModifyDBProxyResponsePrivate(
    ModifyDBProxyResponse * const q) : RdsResponsePrivate(q)
{
...
{
    Q_ASSERT(xml.name() == QLatin1String("ModifyDBProxyResponse"));
    Q_UNUSED(xml) ///< @todo
}
