// SPDX-FileCopyrightText: 2013-2022 Paul Colby <git@colby.id.au>
// SPDX-License-Identifier: LGPL-3.0-or-later

#ifndef QTAWS_DELETEUSERHIERARCHYGROUPRESPONSE_P_H
#define QTAWS_DELETEUSERHIERARCHYGROUPRESPONSE_P_H

#include "connectresponse_p.h"

namespace QtAws {
namespace Connect {

class DeleteUserHierarchyGroupResponse;

class DeleteUserHierarchyGroupResponsePrivate : public ConnectResponsePrivate {

public:

    explicit DeleteUserHierarchyGroupResponsePrivate(DeleteUserHierarchyGroupResponse * const q);

    void parseDeleteUserHierarchyGroupResponse(QXmlStreamReader &xml);

private:
    Q_DECLARE_PUBLIC(DeleteUserHierarchyGroupResponse)
    Q_DISABLE_COPY(DeleteUserHierarchyGroupResponsePrivate)

};

} // namespace Connect
} // namespace QtAws

#endif
