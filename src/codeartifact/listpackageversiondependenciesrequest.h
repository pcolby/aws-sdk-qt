// SPDX-FileCopyrightText: 2013-2022 Paul Colby <git@colby.id.au>
// SPDX-License-Identifier: LGPL-3.0-or-later

#ifndef QTAWS_LISTPACKAGEVERSIONDEPENDENCIESREQUEST_H
#define QTAWS_LISTPACKAGEVERSIONDEPENDENCIESREQUEST_H

#include "codeartifactrequest.h"

namespace QtAws {
namespace CodeArtifact {

class ListPackageVersionDependenciesRequestPrivate;

class QTAWSCODEARTIFACT_EXPORT ListPackageVersionDependenciesRequest : public CodeArtifactRequest {

public:
    ListPackageVersionDependenciesRequest(const ListPackageVersionDependenciesRequest &other);
    ListPackageVersionDependenciesRequest();

    virtual bool isValid() const Q_DECL_OVERRIDE;


protected:
    virtual QtAws::Core::AwsAbstractResponse * response(QNetworkReply * const reply) const Q_DECL_OVERRIDE;

private:
    Q_DECLARE_PRIVATE(ListPackageVersionDependenciesRequest)

};

} // namespace CodeArtifact
} // namespace QtAws

#endif
