...
#include "batchsuspenduserresponse_p.h"
...
/*!
 * \class QtAws::Chime::BatchSuspendUserResponse
 * \brief The BatchSuspendUserResponse class provides an interace for Chime BatchSuspendUser responses.
 *
 * \inmodule QtAwsChime
 *
 ...
 *
 * \sa ChimeClient::batchSuspendUser
 */

/*!
 * Constructs a BatchSuspendUserResponse object for \a reply to \a request, with parent \a parent.
 */
BatchSuspendUserResponse::BatchSuspendUserResponse(
        const BatchSuspendUserRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : ChimeResponse(new BatchSuspendUserResponsePrivate(this), parent)
{
    setRequest(new BatchSuspendUserRequest(request));
    setReply(reply);
}
 ...
 * \reimp
 */
const BatchSuspendUserRequest * BatchSuspendUserResponse::request() const
{
    Q_D(const BatchSuspendUserResponse);
    return static_cast<const BatchSuspendUserRequest *>(d->request);
}

/*!
 * \reimp
 * Parses a successful Chime BatchSuspendUser \a response.
 */
void BatchSuspendUserResponse::parseSuccess(QIODevice &response)
{
    //Q_D(BatchSuspendUserResponse);
    QXmlStreamReader xml(&response);
    /// @todo
}

/*!
 * \class QtAws::Chime::BatchSuspendUserResponsePrivate
 * \brief The BatchSuspendUserResponsePrivate class provides private implementation for BatchSuspendUserResponse.
 * \internal
 *
 * \inmodule QtAwsChime
 */
...
/*!
 * Constructs a BatchSuspendUserResponsePrivate object with public implementation \a q.
 */
BatchSuspendUserResponsePrivate::BatchSuspendUserResponsePrivate(
    BatchSuspendUserResponse * const q) : ChimeResponsePrivate(q)
{

}

/*!
 * Parses a Chime BatchSuspendUser response element from \a xml.
 */
void BatchSuspendUserResponsePrivate::parseBatchSuspendUserResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("BatchSuspendUserResponse"));
    Q_UNUSED(xml) ///< @todo
}
