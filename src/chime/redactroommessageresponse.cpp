...
#include "redactroommessageresponse_p.h"
...
/*!
 * \class QtAws::Chime::RedactRoomMessageResponse
 * \brief The RedactRoomMessageResponse class provides an interace for Chime RedactRoomMessage responses.
 *
 * \inmodule QtAwsChime
 *
...
/*!
 * Constructs a RedactRoomMessageResponse object for \a reply to \a request, with parent \a parent.
 */
RedactRoomMessageResponse::RedactRoomMessageResponse(
        const RedactRoomMessageRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : ChimeResponse(new RedactRoomMessageResponsePrivate(this), parent)
{
    setRequest(new RedactRoomMessageRequest(request));
    setReply(reply);
}
...
/*!
 * \reimp
 * Parses a successful Chime RedactRoomMessage \a response.
 */
void RedactRoomMessageResponse::parseSuccess(QIODevice &response)
{
    //Q_D(RedactRoomMessageResponse);
    QXmlStreamReader xml(&response);
    /// @todo
}

/*!
 * \class QtAws::Chime::RedactRoomMessageResponsePrivate
 * \brief The RedactRoomMessageResponsePrivate class provides private implementation for RedactRoomMessageResponse.
 * \internal
 *
 * \inmodule QtAwsChime
 */
...
/*!
 * Constructs a RedactRoomMessageResponsePrivate object with public implementation \a q.
 */
RedactRoomMessageResponsePrivate::RedactRoomMessageResponsePrivate(
    RedactRoomMessageResponse * const q) : ChimeResponsePrivate(q)
{

}

/*!
 * Parses a Chime RedactRoomMessage response element from \a xml.
 */
void RedactRoomMessageResponsePrivate::parseRedactRoomMessageResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("RedactRoomMessageResponse"));
    Q_UNUSED(xml) ///< @todo
}
