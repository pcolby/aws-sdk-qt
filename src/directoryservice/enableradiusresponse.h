...
class QTAWSDIRECTORYSERVICE_EXPORT EnableRadiusResponse : public DirectoryServiceResponse {
    Q_OBJECT

public:
    EnableRadiusResponse(const EnableRadiusRequest &request, QNetworkReply * const reply, QObject * const parent = 0);

    virtual const EnableRadiusRequest * request() const Q_DECL_OVERRIDE;
