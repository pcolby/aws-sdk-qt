 ...
 * email> </b>
 *
 * If you only specify the <code>SourceIdentityArn</code> parameter, Amazon SES sets the From and Return-Path addresses to
 * the same identity that you
 *
 * specified>
 *
 ...
 * \note The caller is to take responsbility for the resulting pointer.
 *
 * Enables or disables Easy DKIM signing of email sent from an identity. If Easy DKIM signing is enabled for a domain, then
 * Amazon SES uses DKIM to sign all email that it sends from addresses on that domain. If Easy DKIM signing is enabled for
 * an email address, then Amazon SES uses DKIM to sign all email it sends from that
 *
 * address> <note>
 *
 * For email addresses (for example, <code>user@example.com</code>), you can only enable DKIM signing if the corresponding
 * domain (in this case, <code>example.com</code>) has been set up to use Easy
 *
 * DKIM> </note>
 *
 * You can enable DKIM signing for an identity at any time after you start the verification process for the identity, even
 * if the verification process isn't complete.
 *
 * </p
 *
 * You can execute this operation no more than once per
