 ...
 *
 * This is the Amazon DynamoDB API Reference. This guide provides descriptions of the low-level DynamoDB
 *
 * API>
 *
 ...
 * Guide.> </li> </ul>
 *
 * Instead of making the requests to the low-level DynamoDB API directly from your application, we recommend that you use
 * the AWS Software Development Kits (SDKs). The easy-to-use libraries in the AWS SDKs make it unnecessary to call the
 * low-level DynamoDB API directly from your application. The libraries take care of request authentication, serialization,
 ...
 * DynamoDB</a> in the Amazon DynamoDB Developer
 *
 * Guide>
 *
 ...
 * </p <ul> <li>
 *
 * <i>CreateTable</i> - Creates a table with user-specified provisioned throughput settings. You must define a primary key
 * for the table - either a simple primary key (partition key), or a composite primary key (partition key and sort key).
 * Optionally, you can create one or more secondary indexes, which provide fast data access using non-key
 *
 * attributes> </li> <li>
 *
 * <i>DescribeTable</i> - Returns metadata for a table, such as table size, status, and index
 *
 * information> </li> <li>
 *
 * <i>UpdateTable</i> - Modifies the provisioned throughput settings for a table. Optionally, you can modify the
 * provisioned throughput settings for global secondary indexes on the
 *
 * table> </li> <li>
 *
 * <i>ListTables</i> - Returns a list of all tables associated with the current AWS account and
 *
 * endpoint> </li> <li>
 *
 * <i>DeleteTable</i> - Deletes a table and all of its
 *
 * indexes> </li> </ul>
 *
 * For conceptual information about managing tables, see <a
 * href="http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithTables.html">Working with Tables</a> in
 ...
 * performs an eventually consistent read; however, applications can request a strongly consistent read
 *
 * instead> </li> <li>
 *
 * <i>BatchGetItem</i> - Performs multiple <i>GetItem</i> requests for data items using their primary keys, from one table
 * or multiple tables. The response from <i>BatchGetItem</i> has a size limit of 16 MB and returns a maximum of 100 items.
 * Both eventually consistent and strongly consistent reads can be
 *
 * used> </li> <li>
 *
 ...
 * index key. <i>Query</i> supports either eventual or strong consistency. A single response has a size limit of 1
 *
 * MB> </li> <li>
 *
 ...
 * <i>Scan</i> should not be used for any application query use case that requires predictable
 *
 * performance> </li> </ul>
 *
 * For conceptual information about reading data, see <a
 * href="http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/WorkingWithItems.html">Working with Items</a> and
 * <a href="http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/QueryAndScan.html">Query and Scan
 * Operations</a> in the <i>Amazon DynamoDB Developer
 *
 * Guide</i>>
 *
 * <b>Modifying Data</b>
 ...
 * default, if an item in the table already exists with the same primary key, the new item completely replaces the existing
 * item. You can use conditional operators to replace an item only if its attribute values match certain conditions, or to
 * insert a new item only if that item doesn't already
 *
 * exist> </li> <li>
 *
 * <i>UpdateItem</i> - Modifies the attributes of an existing item. You can also use conditional operators to perform an
 * update only if the item's attribute values match certain
 *
 * conditions> </li> <li>
 *
 * <i>DeleteItem</i> - Deletes an item in a table by primary key. You can use conditional operators to perform a delete an
 * item only if the item's attribute values match certain
 *
 * conditions> </li> <li>
 *
