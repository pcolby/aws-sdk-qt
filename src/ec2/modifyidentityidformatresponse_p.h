// SPDX-FileCopyrightText: 2013-2022 Paul Colby <git@colby.id.au>
// SPDX-License-Identifier: LGPL-3.0-or-later

#ifndef QTAWS_MODIFYIDENTITYIDFORMATRESPONSE_P_H
#define QTAWS_MODIFYIDENTITYIDFORMATRESPONSE_P_H

#include "ec2response_p.h"

namespace QtAws {
namespace Ec2 {

class ModifyIdentityIdFormatResponse;

class ModifyIdentityIdFormatResponsePrivate : public Ec2ResponsePrivate {

public:

    explicit ModifyIdentityIdFormatResponsePrivate(ModifyIdentityIdFormatResponse * const q);

    void parseModifyIdentityIdFormatResponse(QXmlStreamReader &xml);

private:
    Q_DECLARE_PUBLIC(ModifyIdentityIdFormatResponse)
    Q_DISABLE_COPY(ModifyIdentityIdFormatResponsePrivate)

};

} // namespace Ec2
} // namespace QtAws

#endif
