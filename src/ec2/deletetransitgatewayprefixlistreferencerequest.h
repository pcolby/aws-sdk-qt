// SPDX-FileCopyrightText: 2013-2022 Paul Colby <git@colby.id.au>
// SPDX-License-Identifier: LGPL-3.0-or-later

#ifndef QTAWS_DELETETRANSITGATEWAYPREFIXLISTREFERENCEREQUEST_H
#define QTAWS_DELETETRANSITGATEWAYPREFIXLISTREFERENCEREQUEST_H

#include "ec2request.h"

namespace QtAws {
namespace Ec2 {

class DeleteTransitGatewayPrefixListReferenceRequestPrivate;

class QTAWSEC2_EXPORT DeleteTransitGatewayPrefixListReferenceRequest : public Ec2Request {

public:
    DeleteTransitGatewayPrefixListReferenceRequest(const DeleteTransitGatewayPrefixListReferenceRequest &other);
    DeleteTransitGatewayPrefixListReferenceRequest();

    virtual bool isValid() const Q_DECL_OVERRIDE;


protected:
    virtual QtAws::Core::AwsAbstractResponse * response(QNetworkReply * const reply) const Q_DECL_OVERRIDE;

private:
    Q_DECLARE_PRIVATE(DeleteTransitGatewayPrefixListReferenceRequest)

};

} // namespace Ec2
} // namespace QtAws

#endif
