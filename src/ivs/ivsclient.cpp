...
/*!
 * \namespace QtAws::IVS
 * \brief Contains classess for accessing Amazon Interactive Video Service ( IVS).
 *
 * \inmodule QtAwsIvs
 *
 ...
 * \brief The IvsClient class provides access to the Amazon Interactive Video Service ( IVS) service.
 *
 * \ingroup aws-clients
 * \inmodule QtAwsIVS
 *
 ...
 * S3>
 *
 * <b>Known issue:</b> In the us-east-1 region, if you use the AWS CLI to create a recording configuration, it returns
 * success even if the S3 bucket is in a different region. In this case, the <code>state</code> of the recording
 ...
 * \note The caller is to take responsbility for the resulting pointer.
 *
 * Gets summary information about all channels in your account, in the AWS region where the API request is processed. This
 * list can be filtered to match a specified name or recording-configuration ARN. Filters are mutually exclusive and cannot
 * be used together. If you try to use both filters, you will get an error (409
 */
 ...
 * \note The caller is to take responsbility for the resulting pointer.
 *
 * Gets summary information about all recording configurations in your account, in the AWS region where the API request is
 */
 ...
 * \note The caller is to take responsbility for the resulting pointer.
 *
 * Inserts metadata into the active stream of the specified channel. A maximum of 5 requests per second per channel is
 * allowed, each with a maximum 1 KB payload. (If 5 TPS is not sufficient for your needs, we recommend batching your data
 * into a single PutMetadata call.) Also see <a
