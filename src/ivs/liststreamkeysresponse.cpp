...
#include "liststreamkeysresponse_p.h"
...
/*!
 * Constructs a ListStreamKeysResponse object for \a reply to \a request, with parent \a parent.
 */
ListStreamKeysResponse::ListStreamKeysResponse(
        const ListStreamKeysRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : IvsResponse(new ListStreamKeysResponsePrivate(this), parent)
{
    setRequest(new ListStreamKeysRequest(request));
    setReply(reply);
}
...
/*!
 * Constructs a ListStreamKeysResponsePrivate object with public implementation \a q.
 */
ListStreamKeysResponsePrivate::ListStreamKeysResponsePrivate(
    ListStreamKeysResponse * const q) : IvsResponsePrivate(q)
{
...
{
    Q_ASSERT(xml.name() == QLatin1String("ListStreamKeysResponse"));
    Q_UNUSED(xml) ///< @todo
}
