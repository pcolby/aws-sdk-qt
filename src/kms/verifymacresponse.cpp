...
/*!
 * \class QtAws::Kms::VerifyMacResponse
 * \brief The VerifyMacResponse class provides an interace for Kms VerifyMac responses.
 *
 * \inmodule QtAwsKms
 *
 ...
 *
 * \sa KmsClient::verifyMac
 */

/*!
 * Constructs a VerifyMacResponse object for \a reply to \a request, with parent \a parent.
 */
VerifyMacResponse::VerifyMacResponse(
        const VerifyMacRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : KmsResponse(new VerifyMacResponsePrivate(this), parent)
{
    setRequest(new VerifyMacRequest(request));
    setReply(reply);
}
 ...
 * \reimp
 */
const VerifyMacRequest * VerifyMacResponse::request() const
{
    Q_D(const VerifyMacResponse);
    return static_cast<const VerifyMacRequest *>(d->request);
}

/*!
 * \reimp
 * Parses a successful Kms VerifyMac \a response.
 */
void VerifyMacResponse::parseSuccess(QIODevice &response)
{
    //Q_D(VerifyMacResponse);
    QXmlStreamReader xml(&response);
    /// @todo
}

/*!
 * \class QtAws::Kms::VerifyMacResponsePrivate
 * \brief The VerifyMacResponsePrivate class provides private implementation for VerifyMacResponse.
 * \internal
 *
 * \inmodule QtAwsKms
 */
...
/*!
 * Constructs a VerifyMacResponsePrivate object with public implementation \a q.
 */
VerifyMacResponsePrivate::VerifyMacResponsePrivate(
    VerifyMacResponse * const q) : KmsResponsePrivate(q)
{

}

/*!
 * Parses a Kms VerifyMac response element from \a xml.
 */
void VerifyMacResponsePrivate::parseVerifyMacResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("VerifyMacResponse"));
    Q_UNUSED(xml) ///< @todo
}
