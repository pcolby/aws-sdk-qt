...
#include "generatemacresponse_p.h"
...
/*!
 * \class QtAws::Kms::GenerateMacResponse
 * \brief The GenerateMacResponse class provides an interace for Kms GenerateMac responses.
 *
 * \inmodule QtAwsKms
 *
 ...
 *
 * \sa KmsClient::generateMac
 */

/*!
 * Constructs a GenerateMacResponse object for \a reply to \a request, with parent \a parent.
 */
GenerateMacResponse::GenerateMacResponse(
        const GenerateMacRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : KmsResponse(new GenerateMacResponsePrivate(this), parent)
{
    setRequest(new GenerateMacRequest(request));
    setReply(reply);
}
 ...
 * \reimp
 */
const GenerateMacRequest * GenerateMacResponse::request() const
{
    Q_D(const GenerateMacResponse);
    return static_cast<const GenerateMacRequest *>(d->request);
}

/*!
 * \reimp
 * Parses a successful Kms GenerateMac \a response.
 */
void GenerateMacResponse::parseSuccess(QIODevice &response)
{
    //Q_D(GenerateMacResponse);
    QXmlStreamReader xml(&response);
    /// @todo
}

/*!
 * \class QtAws::Kms::GenerateMacResponsePrivate
 * \brief The GenerateMacResponsePrivate class provides private implementation for GenerateMacResponse.
 * \internal
 *
 * \inmodule QtAwsKms
 */
...
/*!
 * Constructs a GenerateMacResponsePrivate object with public implementation \a q.
 */
GenerateMacResponsePrivate::GenerateMacResponsePrivate(
    GenerateMacResponse * const q) : KmsResponsePrivate(q)
{

}

/*!
 * Parses a Kms GenerateMac response element from \a xml.
 */
void GenerateMacResponsePrivate::parseGenerateMacResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("GenerateMacResponse"));
    Q_UNUSED(xml) ///< @todo
}
