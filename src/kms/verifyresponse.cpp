...
/*!
 * \class QtAws::KMS::VerifyResponse
 * \brief The VerifyResponse class provides an interace for KMS Verify responses.
 *
 * \inmodule QtAwsKMS
 *
...
/*!
 * Constructs a VerifyResponse object for \a reply to \a request, with parent \a parent.
 */
VerifyResponse::VerifyResponse(
        const VerifyRequest &request,
        QNetworkReply * const reply,
        QObject * const parent)
    : KmsResponse(new VerifyResponsePrivate(this), parent)
{
    setRequest(new VerifyRequest(request));
    setReply(reply);
}
...
/*!
 * \reimp
 * Parses a successful KMS Verify \a response.
 */
void VerifyResponse::parseSuccess(QIODevice &response)
{
    //Q_D(VerifyResponse);
...
/*!
 * \class QtAws::KMS::VerifyResponsePrivate
 * \brief The VerifyResponsePrivate class provides private implementation for VerifyResponse.
 * \internal
 *
 * \inmodule QtAwsKMS
 */
...
/*!
 * Constructs a VerifyResponsePrivate object with public implementation \a q.
 */
VerifyResponsePrivate::VerifyResponsePrivate(
    VerifyResponse * const q) : KmsResponsePrivate(q)
{

}

/*!
 * Parses a KMS Verify response element from \a xml.
 */
void VerifyResponsePrivate::parseVerifyResponse(QXmlStreamReader &xml)
{
    Q_ASSERT(xml.name() == QLatin1String("VerifyResponse"));
    Q_UNUSED(xml) ///< @todo
}
