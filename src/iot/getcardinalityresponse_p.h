// SPDX-FileCopyrightText: 2013-2022 Paul Colby <git@colby.id.au>
// SPDX-License-Identifier: LGPL-3.0-or-later

#ifndef QTAWS_GETCARDINALITYRESPONSE_P_H
#define QTAWS_GETCARDINALITYRESPONSE_P_H

#include "iotresponse_p.h"

namespace QtAws {
namespace IoT {

class GetCardinalityResponse;

class GetCardinalityResponsePrivate : public IoTResponsePrivate {

public:

    explicit GetCardinalityResponsePrivate(GetCardinalityResponse * const q);

    void parseGetCardinalityResponse(QXmlStreamReader &xml);

private:
    Q_DECLARE_PUBLIC(GetCardinalityResponse)
    Q_DISABLE_COPY(GetCardinalityResponsePrivate)

};

} // namespace IoT
} // namespace QtAws

#endif
