 ...
 * <a>PutRecord</a> or <a>PutRecordBatch</a>, or it can be configured to use an existing Kinesis data stream as its source.
 * To specify a Kinesis data stream as input, set the <code>DeliveryStreamType</code> parameter to
 * <code>KinesisStreamAsSource</code>, and provide the Kinesis data stream Amazon Resource Name (ARN) and role ARN in the
 * <code>KinesisStreamSourceConfiguration</code>
 *
 * parameter>
 *
 * A delivery stream is configured with a single destination: Amazon S3, Amazon ES, Amazon Redshift, or Splunk. Specify
 * only one of the following destination configuration parameters: <code>ExtendedS3DestinationConfiguration</code>,
 ...
 * <code>BufferingHints</code>, <code>EncryptionConfiguration</code>, and <code>CompressionFormat</code>. By default, if no
 ...
 * adhere to these conditions strictly. For example, record boundaries are such that the size is a little over or under the
 ...
 * destination> <ul> <li>
 *
 * An Amazon Redshift destination requires an S3 bucket as intermediate location. This is because Kinesis Data Firehose
 * first delivers data to Amazon S3 and then uses <code>COPY</code> syntax to load data into an Amazon Redshift table. This
 * is specified in the <code>RedshiftDestinationConfiguration.S3Configuration</code>
 *
 * parameter> </li> <li>
 *
 ...
 * formats> </li> <li>
 *
 * We strongly recommend that you use the user name and password that you provide exclusively with Kinesis Data Firehose.
 ...
 * href="http://docs.aws.amazon.com/firehose/latest/dev/controlling-access.html#using-iam-s3">Grant Kinesis Firehose Access
 * to an Amazon S3 Destination</a> in the <i>Amazon Kinesis Data Firehose Developer
 */
 ...
 * can limit the number of delivery streams returned, using the <b>Limit</b> parameter. To determine whether there are more
 * delivery streams to list, check the value of <code>HasMoreDeliveryStreams</code> in the output. If there are more
 * delivery streams to list, you can request them by specifying the name of the last delivery stream returned in the call
 * in the <code>ExclusiveStartDeliveryStreamName</code> parameter of a subsequent
 */
 ...
 * second. Note that if you use <code>PutRecord</code> and <a>PutRecordBatch</a>, the limits are an aggregate across these
 ...
 * </p
 *
 * You must specify the name of the delivery stream and the data record when using <code>PutRecord</code>. The data record
 * consists of a data blob that can be up to 1,000 KB in size and any kind of data. For example, it can be a segment from a
 ...
 *
 * By default, each delivery stream can take in up to 2,000 transactions per second, 5,000 records per second, or 5 MB per
 * second. If you use <a>PutRecord</a> and <code>PutRecordBatch</code>, the limits are an aggregate across these two
 ...
 * Limits</a>>
 *
 * Each <code>PutRecordBatch</code> request supports up to 500 records. Each record in the request can be as large as 1,000
 * KB (before 64-bit encoding), up to a limit of 4 MB for the entire request. These limits cannot be
 *
 * changed>
 *
 * You must specify the name of the delivery stream and the data record when using <a>PutRecord</a>. The data record
 * consists of a data blob that can be up to 1,000 KB in size and any kind of data. For example, it could be a segment from
 ...
 * destination>
 *
 * The <code>PutRecordBatch</code> response includes a count of failed records, <code>FailedPutCount</code>, and an array
 * of responses, <code>RequestResponses</code>. Each entry in the <code>RequestResponses</code> array provides additional
 * information about the processed record. It directly correlates with a record in the request array using the same
 * ordering, from the top to the bottom. The response array always includes the same number of records as the request
 * array. <code>RequestResponses</code> includes both successfully and unsuccessfully processed records. Kinesis Data
 * Firehose attempts to process all records in each <code>PutRecordBatch</code> request. A single record failure does not
 * stop the processing of subsequent
 *
 * records>
 *
 ...
 * <code>ErrorCode</code> reflects the type of error, and is one of the following values: <code>ServiceUnavailable</code>
 * or <code>InternalFailure</code>. <code>ErrorMessage</code> provides more detailed information about the
 *
 * error>
 *
 ...
 * destination>
 *
 * If <code>PutRecordBatch</code> throws <code>ServiceUnavailableException</code>, back off and retry. If the exception
 * persists, it is possible that the throughput limits have been exceeded for the delivery
 *
 * stream>
 *
 * Data records sent to Kinesis Data Firehose are stored for 24 hours from the time they are added to a delivery stream as
 ...
 * \note The caller is to take responsbility for the resulting pointer.
 *
 * Adds or updates tags for the specified delivery stream. A tag is a key-value pair (the value is optional) that you can
 * define and assign to AWS resources. If you specify a tag that already exists, the tag value is replaced with the value
 * that you specify in the request. Tags are metadata. For example, you can add friendly names and descriptions or other
 ...
 * that matches. After the update is applied successfully, the version ID is updated, and you can retrieve it using
