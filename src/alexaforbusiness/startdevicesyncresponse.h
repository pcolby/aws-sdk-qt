...
class QTAWSALEXAFORBUSINESS_EXPORT StartDeviceSyncResponse : public AlexaForBusinessResponse {
    Q_OBJECT

public:
    StartDeviceSyncResponse(const StartDeviceSyncRequest &request, QNetworkReply * const reply, QObject * const parent = 0);

    virtual const StartDeviceSyncRequest * request() const Q_DECL_OVERRIDE;
