...
class QTAWSSECRETSMANAGER_EXPORT PutSecretValueResponse : public SecretsManagerResponse {
    Q_OBJECT

public:
    PutSecretValueResponse(const PutSecretValueRequest &request, QNetworkReply * const reply, QObject * const parent = 0);

    virtual const PutSecretValueRequest * request() const Q_DECL_OVERRIDE;
